...
# Story 28.4: Update Test Infrastructure for Public Testnets
...
**Task Execution Strategy:** This story updates `docker-compose-agent-test.yml` and `scripts/run-docker-agent-test.sh` to support the `NETWORK_MODE` environment variable from Story 28.3. Local blockchain containers are moved to a profile so they only start in local mode.
...
This is Story 28.4 (revised) in Epic 28: Public Testnet Integration. The original 28.4 focused on integrating the multi-arch Docker image; this replacement updates the infrastructure for public testnet support.
...
- **Story 28.4 (this story)**: Update Test Infrastructure for Testnets
- **Story 28.5**: Production Connector Aptos Settlement

**Dependencies:**

- Story 28.3 must be complete (provides testnet URL configuration in test runner)
...
- Testnet faucets have rate limits (Story 28.3 implements retry logic)
...
│       ├── src/
│       │   └── test/
│       │       ├── docker-agent-test-runner.ts  # Test orchestrator (Story 28.3)
│       │       └── testnet-config.ts            # URL configuration (Story 28.3)
