...
- **Important:** For Story 2.5, peer configuration will be via environment variables (BTP_PEER_{ID}_URL, BTP_PEER_{ID}_SECRET)
...
describe('Docker Compose Multi-Node Deployment', () => {
  ...
  it('should start all 3 connectors successfully', async () => {
    ...
    // Verify all healthy
    const healthStatus = await exec('docker inspect --format="{{.State.Health.Status}}" connector-a connector-b connector-c');
...
4. ✅ BTP peer connection environment variables documented (BTP_PEER_*_URL, BTP_PEER_*_SECRET)
