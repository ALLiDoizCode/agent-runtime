...
- [ ] Task 1: Create Default Linear Topology Docker Compose Configuration (AC: 1, 2, 3, 4, 5, 6, 7)
  ...
  - [ ] Define service `connector-a` using `agent-runtime` image:
  ...
  - [ ] Define service `connector-b` using `agent-runtime` image:
  ...
  - [ ] Define service `connector-c` using `agent-runtime` image:
...
- [ ] Each connector service uses `agent-runtime` image
