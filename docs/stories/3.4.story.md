...
class TelemetryEmitter {
  ...
  async connect(): Promise<void> {
    try {
      ...
      this._ws.on('error', (error) => {
        this._logger.warn({ event: 'telemetry_error', error: error.message }, 'Telemetry connection error');
      ...
      this._logger.warn({ event: 'telemetry_connect_failed', error }, 'Failed to connect telemetry');
      this._scheduleReconnect();
    }
  }
  ...
  private _sendTelemetry(message: TelemetryMessage): void {
    if (!this._connected || !this._ws) {
      this._logger.debug({ event: 'telemetry_not_connected' }, 'Telemetry not connected, skipping emission');
      return;
    }
