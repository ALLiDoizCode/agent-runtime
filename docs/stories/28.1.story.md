...
**Task Execution Strategy:** Story 28.1 adds Aptos payment channel endpoints to the existing agent-server.ts, mirroring the established patterns for EVM (Story 8.x) and XRP (Story 9.x) payment channels. The implementation follows the existing endpoint patterns in agent-server.ts and integrates with the AptosChannelSDK created in Epic 27 (Story 27.4). Tasks are ordered to build incrementally: types first, initialization, then endpoints, and finally tests.
...
- [x] Task 1: Add Aptos Types and Configuration to AgentServer (AC: 7)
  ...
  - [x] Extend `AgentServerConfig` interface with Aptos configuration:
    ...
    - [Source: Epic 28 Technical Approach - Story 28.1]
  ...
  - [x] Load Aptos configuration from environment in constructor:
    ...
    - [Source: Epic 28 Technical Approach - Environment configuration]
...
This is the **first story in Epic 28: Complete Aptos Payment Channel Integration**. Epic 28 completes the Aptos payment channel integration across all system layers to enable tri-chain settlement (EVM + XRP + Aptos).

**Epic 28 Context:**

- **Story 28.1 (this story)**: Add Agent Server Aptos HTTP Endpoints
- **Story 28.2**: Add Test Runner Aptos Phases
- **Story 28.3**: Build Multi-Arch Aptos Docker Image
- **Story 28.4**: Integrate Aptos into Test Infrastructure
- **Story 28.5**: Production Connector Aptos Settlement

**Architectural Role:**

Story 28.1 **adds Aptos payment channel HTTP endpoints to the agent server**, mirroring the existing EVM and XRP endpoint patterns. This enables agents to manage Aptos payment channels via HTTP API, which is required for the Docker agent test runner (Story 28.2) to orchestrate Aptos test phases.

**Foundation from Epic 27:**

Epic 27 implemented the core Aptos payment channel components:

- Move smart contracts (Story 27.1-27.2)
- TypeScript SDK (Story 27.3-27.4)
- AptosChannelSDK with channel lifecycle methods (Story 27.4)

Story 28.1 wraps the AptosChannelSDK with HTTP endpoints, making it accessible to external clients and the test runner.

### Previous Story Insights

**No previous stories in Epic 28** - this is the first story.

**Relevant patterns from Epic 27 (Aptos SDK):**
...
[Source: Epic 28 Technical Approach]
...
- `packages/connector/src/settlement/aptos-channel-sdk.ts` - AptosChannelSDK (Epic 27.4)
...
Story 28.1 focuses on unit testing the new HTTP endpoints with mocked SDK dependencies.
...
[Source: Aptos documentation, Epic 27]
...
### Out of Scope for Story 28.1

**Explicitly NOT included in this story:**

1. **Test Runner Phases** - Story 28.2 implements Aptos test phases
2. **Docker Image Build** - Story 28.3 builds multi-arch Aptos image
3. **Integration with docker-compose** - Story 28.4 updates test infrastructure
4. **Production Settlement Flow** - Story 28.5 integrates with connector settlement
5. **Aptos Faucet Integration** - Test runner (28.2) handles funding
6. **Move Module Deployment** - Already deployed in Epic 27
7. **Aptos Balance Querying via REST** - Simplified for MVP (placeholder in getBalances)

### Success Criteria

**Story 28.1 is successful when:**
...
The implementation of Story 28.1 demonstrates high-quality code that follows established patterns consistently. The Aptos payment channel HTTP endpoints mirror the existing EVM and XRP patterns exactly, maintaining architectural consistency across all three blockchain integrations.
...
| ---------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------- |
| 2026-01-31 | 1.0     | Initial story creation with comprehensive technical details from Epic 28 requirements, agent-server.ts patterns (EVM/XRP endpoints), AptosChannelSDK from Epic 27, coding standards, and testing strategy. Story focuses on adding Aptos HTTP endpoints mirroring existing payment channel patterns.                                                                                                                                                                                       | Claude (Story Creation Agent) |
