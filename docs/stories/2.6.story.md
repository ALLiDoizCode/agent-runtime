...
- [x] Task 5: Update Docker Compose to Use YAML Configurations (AC: 6, 8)
  ...
  - [ ] Remove old environment variable configuration (BTP_PEER_* pattern) - now handled by YAML
...
- Docker Compose configuration uses environment variables for BTP peer connections (BTP_PEER_{ID}_URL, BTP_PEER_{ID}_SECRET pattern)
