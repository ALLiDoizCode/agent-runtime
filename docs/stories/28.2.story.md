...
**Task Execution Strategy:** Story 28.2 adds Aptos payment channel test phases to the existing `docker-agent-test-runner.ts`, mirroring the established patterns for EVM (lines 614-920) and XRP (lines 929-1479) payment channel testing. The implementation follows the existing phase structure and integrates with the Aptos HTTP endpoints added in Story 28.1. Tasks are ordered to build incrementally: types/config first, then funding, configuration, channel operations, and finally verification phases.
...
This is the **second story in Epic 28: Complete Aptos Payment Channel Integration**. Epic 28 completes the Aptos payment channel integration across all system layers to enable tri-chain settlement (EVM + XRP + Aptos).

**Epic 28 Context:**

- **Story 28.1** (Complete): Add Agent Server Aptos HTTP Endpoints
- **Story 28.2 (this story)**: Add Test Runner Aptos Phases
...
Story 28.2 **adds Aptos payment channel test phases to the Docker agent test runner**, enabling end-to-end validation of Aptos channels alongside EVM and XRP. This ensures the Aptos HTTP endpoints (Story 28.1) work correctly in the full integration test environment.

**Dependency:**

Story 28.2 depends on Story 28.1 (Aptos HTTP endpoints) being complete. The test runner will call the endpoints implemented in 28.1:
...
**Story 28.1 Completion Notes:**

Story 28.1 added the following Aptos HTTP endpoints to agent-server.ts:
...
- `packages/connector/src/agent/agent-server.ts` - Aptos HTTP endpoints (Story 28.1)
- `packages/connector/src/settlement/aptos-channel-sdk.ts` - Aptos SDK (Epic 27)
...
Story 28.2 is primarily validated through the integration test itself. The test phases should:
...
- **Mitigation Options:**
  1. **Option A (Preferred):** Add `aptosPublicKey` to `/status` response in agent-server.ts (minor update to Story 28.1)
...
### Out of Scope for Story 28.2

**Explicitly NOT included in this story:**

1. **Agent Server Endpoints** - Already implemented in Story 28.1
2. **Docker Image Build** - Story 28.3 builds multi-arch Aptos image
3. **docker-compose Updates** - Story 28.4 updates test infrastructure
4. **Production Connector** - Story 28.5 integrates with settlement flow
...
| ---------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- |
| 2026-01-31 | 1.0     | Initial story creation with comprehensive technical details from Epic 28 requirements, docker-agent-test-runner.ts patterns (EVM/XRP phases), Story 28.1 Aptos endpoints, coding standards, and testing strategy. Story focuses on adding Aptos test phases mirroring existing payment channel patterns.                                                                                                                                                                   | Claude (Story Creation Agent) |
