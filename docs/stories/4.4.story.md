...
- [ ] Task 1: Create Send Packet Tool Project Structure (AC: 1)
  ...
  - [ ] Create `tools/send-packet/package.json`:
    ...
    - [ ] dependencies:
      - [ ] "@agent-runtime/shared": "workspace:\*" (for ILP types and OER encoding)
...
- [ ] Task 2: Implement ILP Packet Creation Functions (AC: 4)
  ...
  - [ ] Implement `createTestPreparePacket(destination: string, amount: bigint, expirySeconds: number): ILPPreparePacket`:
    ...
    - [ ] Validate destination address using isValidILPAddress from @agent-runtime/shared
  ...
  - [ ] Implement `createTestFulfillPacket(preimage: Buffer, data?: Buffer): ILPFulfillPacket`:
    - [ ] Import ILPFulfillPacket, PacketType from @agent-runtime/shared
  ...
  - [ ] Implement `createTestRejectPacket(code: ILPErrorCode, message: string, triggeredBy: string, data?: Buffer): ILPRejectPacket`:
    - [ ] Import ILPRejectPacket, PacketType, ILPErrorCode from @agent-runtime/shared
...
- [ ] Task 3: Implement BTP Connection and Packet Sending Logic (AC: 3, 5)
  ...
  - [ ] Implement `BTPSender` class based on BTPClient pattern:
    ...
    - [ ] Method: `sendPacket(packet: ILPPreparePacket): Promise<ILPFulfillPacket | ILPRejectPacket>`
      ...
      - [ ] Serialize ILP packet using serializePacket from @agent-runtime/shared
...
1. Serialize ILP packet using serializePacket from @agent-runtime/shared
...
- Dependencies:
  - @agent-runtime/shared - ILP types, OER encoding
...
- Uses workspace dependencies (@agent-runtime/shared, @agent-runtime/connector types)
...
- [ ] Consider extracting BTP message parsing to @agent-runtime/shared to reduce code duplication
