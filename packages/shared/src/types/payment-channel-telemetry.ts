...
export interface DashboardChannelState {
  ...
  /** Settlement method discriminator (Story 9.7, Story 27.5) */
  ...
  // Aptos-specific fields (Story 27.5)
  /** Aptos channel owner address (Story 27.5) */
  aptosChannelOwner?: string;
  /** Aptos destination address (Story 27.5) */
  aptosDestination?: string;
  /** Total APT deposited in octas (Story 27.5) */
  aptosDeposited?: string;
  /** APT claimed so far in octas (Story 27.5) */
  aptosClaimed?: string;
  /** Aptos settle delay in seconds (Story 27.5) */
  aptosSettleDelay?: number;
  /** Aptos destination public key (Story 27.5) */
  aptosPubkey?: string;
}

// ============================================================================
// Aptos Settlement Telemetry Events (Story 27.5)
// ============================================================================

/**
 * Aptos Channel Opened Telemetry Event
 *
 * Emitted when a new Aptos payment channel is opened.
 *
 * @example
 * ```typescript
 * const event: AptosChannelOpenedEvent = {
 *   type: 'APTOS_CHANNEL_OPENED',
 *   channelOwner: '0x1234...',
 *   destination: '0xabcd...',
 *   amount: '1000000000',  // 10 APT in octas
 *   settleDelay: 86400,
 *   timestamp: Date.now()
 * };
 * ```
 */
export interface AptosChannelOpenedEvent {
  /** Event type discriminator */
  type: 'APTOS_CHANNEL_OPENED';
  /** Aptos channel owner address (0x-prefixed hex) */
  channelOwner: string;
  /** Aptos destination address (0x-prefixed hex) */
  destination: string;
  /** Initial deposit amount in octas (bigint as string) */
  amount: string;
...
/**
 * Aptos Claim Signed Telemetry Event
 *
 * Emitted when an off-chain Aptos claim is signed.
 *
 * @example
 * ```typescript
 * const event: AptosClaimSignedEvent = {
 ...
 * ```
 */
export interface AptosClaimSignedEvent {
  /** Event type discriminator */
  type: 'APTOS_CLAIM_SIGNED';
  /** Aptos channel owner address (0x-prefixed hex) */
  channelOwner: string;
  /** Cumulative claimed amount in octas (bigint as string) */
  amount: string;
  /** Claim nonce (monotonically increasing) */
  nonce: number;
  /** Event timestamp (Unix milliseconds) */
  timestamp: number;
}
...
/**
 * Aptos Claim Submitted Telemetry Event
 *
 * Emitted when an Aptos claim is submitted on-chain.
 *
 * @example
 * ```typescript
 * const event: AptosClaimSubmittedEvent = {
 ...
 * ```
 */
export interface AptosClaimSubmittedEvent {
  /** Event type discriminator */
  type: 'APTOS_CLAIM_SUBMITTED';
  /** Aptos channel owner address (0x-prefixed hex) */
  channelOwner: string;
...
/**
 * Aptos Settlement Completed Telemetry Event
 *
 * Emitted when Aptos settlement completes successfully.
 *
 * @example
 * ```typescript
 * const event: AptosSettlementCompletedEvent = {
 ...
 * ```
 */
export interface AptosSettlementCompletedEvent {
  /** Event type discriminator */
  type: 'APTOS_SETTLEMENT_COMPLETED';
  /** Peer identifier */
  peerId: string;
  /** Settlement amount in octas (bigint as string) */
  amount: string;
  /** Aptos channel owner address (0x-prefixed hex) */
  channelOwner: string;
  /** Event timestamp (Unix milliseconds) */
...
/**
 * Aptos Settlement Failed Telemetry Event
 *
 * Emitted when Aptos settlement fails.
 *
 * @example
 * ```typescript
 * const event: AptosSettlementFailedEvent = {
 ...
 * ```
 */
export interface AptosSettlementFailedEvent {
  /** Event type discriminator */
  type: 'APTOS_SETTLEMENT_FAILED';
  /** Peer identifier */
  peerId: string;
...
/**
 * Union type for all Aptos settlement telemetry events
 */
