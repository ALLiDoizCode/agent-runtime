  ...
  if (error) {
    return (
      <div className="p-4 text-sm text-red-400">
        ...
        <button onClick={refresh} className="mt-2 text-xs underline hover:no-underline">
          Try again
        </button>
      </div>
    );
    ...
    <div className="flex flex-col gap-2 p-4">
      ...
      {relatedEvents.map((related) => (
        <button
          ...
          className="flex items-center justify-between p-3 rounded-md border border-border hover:bg-muted/50 transition-colors text-left"
        >
              ...
              <div className="flex-1 overflow-y-auto">
                <TabsContent value="raw" className="mt-0 p-4">
                  <div className="flex justify-end mb-2">
                    <CopyButton
                      value={JSON.stringify(displayData.payload, null, 2)}
                      label="Copy JSON"
                    />
                  </div>
